        ...
        {detailsOpen ? (
          <div id="uoaisummary-details" className="flex flex-col gap-3">
            <section className="grid grid-cols-1 gap-2 @4xl:grid-cols-3">
              {data.summary.actions.map((action, index) => (
                <article
                ...
                >
                  ...
                  {action.shopName ? (
                    <span className="mt-2 block text-xs text-theme-500">{action.shopName}</span>
                  ) : null}
                </article>
              ))}
            </section>
  ...
  const refresh = useCallback(
    async (event) => {
      ...
      try {
        ...
        if (response.status === 429) setRefreshError("cooldown");
        await mutate();
        return payload;
      } catch {
